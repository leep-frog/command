					...
					"linux": {
						WantExecuteData: &command.ExecuteData{
							Executable: []string{
								...
								`}`,
								``,
								`{ { type complete > /dev/null 2>&1 ; } && complete -F _custom_autocomplete_for_alias_some-alias -o nosort some-alias ; } || { echo 'shell function "complete" either failed or does not exist; if using zsh, be sure to set up bashcompinit' 1>&2 ; }`,
								``,
							},
						},
					},
