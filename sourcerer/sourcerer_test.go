...
func TestGenerateBinaryNode(t *testing.T) {
	...
	for _, curOS := range []OS{Linux(), Windows()} {
		for _, test := range []struct {
			...
			{
				name:          "fails if shadow dir is not a dir",
				cliTargetName: "leepFrogSource",
				env: map[string]string{
					RootDirectoryEnvVar: "cli-output-dir",
				},
				args: []string{"source", "--shadow-dir", testutil.FilepathAbs(t, "idk")},
				clis: []CLI{
					ToCLI("x", nil),
					ToCLI("l", nil),
					&testCLI{name: "basic", setup: []string{"his", "story"}},
				},
				wantErr: fmt.Errorf(`validation for "shadow-dir" failed: [IsDir] file %q does not exist`, testutil.FilepathAbs(t, "idk")),
				osChecks: map[string]*osCheck{
					osLinux: {
						wantStderr: []string{
							fmt.Sprintf(`validation for "shadow-dir" failed: [IsDir] file %q does not exist`, testutil.FilepathAbs(t, "idk")),
							``,
						},
					},
					osWindows: {
			...
			{
				name:          "generates source file with shadow dir",
				cliTargetName: "leepFrogSource",
				env: map[string]string{
					RootDirectoryEnvVar: "cli-output-dir",
				},
				args: []string{"source", "--shadow-dir", testutil.FilepathAbs(t, "cmd")},
				clis: []CLI{
					ToCLI("x", nil),
					ToCLI("l", nil),
					&testCLI{name: "basic", setup: []string{"his", "story"}},
						...
						wantOsWriteFiles: []*osWriteFileArgs{
							...
							},
							{
								...
								Contents: []string{
									...
									``,
									`  # Run the go-only code`,
									fmt.Sprintf(`  %s execute "$1" $tmpFile "${@:2}"`, testutil.FilepathAbs(t, `cmd`, "artifacts", "leepFrogSource")),
									`  # Return the error code if go code terminated with an error`,
									`  local errorCode=$?`,
									`  if [ $errorCode -ne 0 ]; then return $errorCode; fi`,
									``,
									...
									`function _custom_autocomplete_leepFrogSource {`,
									`  local tFile=$(mktemp)`,
									fmt.Sprintf(`  %s autocomplete ${COMP_WORDS[0]} "$COMP_TYPE" $COMP_POINT "$COMP_LINE" > $tFile`, testutil.FilepathAbs(t, `cmd`, "artifacts", "leepFrogSource")),
									`  local IFS=$'\n'`,
									`  COMPREPLY=( $(cat $tFile) )`,
									`  rm $tFile`,
									`}`,
						...
						wantOsWriteFiles: []*osWriteFileArgs{
							...
							},
							{
								...
								Contents: []string{
									...
									`  $Local:tmpPassthroughArgFile = New-TemporaryFile`,
									`  [IO.File]::WriteAllText($Local:tmpPassthroughArgFile, $commandAst.ToString())`,
									fmt.Sprintf(`  (& %s autocomplete ($commandAst.CommandElements | Select-Object -first 1) --comp-line-file "0" $compPoint $Local:tmpPassthroughArgFile) | ForEach-Object {`, testutil.FilepathAbs(t, `cmd`, "artifacts", "leepFrogSource.exe")),
									`    "$_"`,
									`  }`,
									`}`,
									``,
									...
									`  _setup_for_basic_cli > "$Local:setupTmpFile"`,
									`  Copy-Item "$Local:setupTmpFile" "$Local:setupTmpFile.txt"`,
									fmt.Sprintf(`  & %s execute "basic" $Local:tmpFile "$Local:setupTmpFile.txt" $args`, testutil.FilepathAbs(t, `cmd`, "artifacts", "leepFrogSource.exe")),
									`  # Return error if failed`,
									`  If (!$?) {`,
									`    Write-Error "Go execution failed"`,
									`  } else {`,
									...
									``,
									`  # Run the go-only code`,
									fmt.Sprintf(`  & %s execute "l" $Local:tmpFile $args`, testutil.FilepathAbs(t, `cmd`, "artifacts", "leepFrogSource.exe")),
									`  # Return error if failed`,
									`  If (!$?) {`,
									`    Write-Error "Go execution failed"`,
									`  } else {`,
									...
									``,
									`  # Run the go-only code`,
									fmt.Sprintf(`  & %s execute "leepFrogSource" $Local:tmpFile $args`, testutil.FilepathAbs(t, `cmd`, "artifacts", "leepFrogSource.exe")),
									`  # Return error if failed`,
									`  If (!$?) {`,
									`    Write-Error "Go execution failed"`,
									`  } else {`,
									...
									``,
									`  # Run the go-only code`,
									fmt.Sprintf(`  & %s execute "x" $Local:tmpFile $args`, testutil.FilepathAbs(t, `cmd`, "artifacts", "leepFrogSource.exe")),
									`  # Return error if failed`,
									`  If (!$?) {`,
									`    Write-Error "Go execution failed"`,
									`  } else {`,
		...
		for _, test := range []struct {
			...
			},
			{
				...
				runResponseTester: &spycommandertest.RunResponseTester{
					...
					Want: []*commandtest.RunContents{{
						Name: "go",
						Args: []string{"run", ".", "source", "--shadow-dir", testutil.FilepathAbs(t, "cli-output-dir")},
			...
			},
			{
				...
				runResponseTester: &spycommandertest.RunResponseTester{
					...
					Want: []*commandtest.RunContents{{
						Name: "go",
						Args: []string{"run", ".", "builtin", "source", "--shadow-dir", testutil.FilepathAbs(t, "cli-output-dir")},
						Dir:  filepath.Dir("/fake/source/location/main.go"),
					}},
				},
			},
			{
				...
				runResponseTester: &spycommandertest.RunResponseTester{
					...
					Want: []*commandtest.RunContents{{
						Name: "go",
						Args: []string{"run", ".", "source", "--shadow-dir", testutil.FilepathAbs(t, "cli-output-dir"), "--quiet"},
						Dir:  filepath.Dir("/fake/source/location/main.go"),
					}},
				},
			},
