	...
	for _, test := range []struct {
		...
		},
		{
			...
			opts: []Option{
				...
				NewAliaser("otherAlias", "all args --at once"),
			},
			wantExecuteFile: []string{
				`function _custom_execute_leep-frog-source {`,
				`  # tmpFile is the file to which we write ExecuteData.Executable`,
			...
			wantStdout: []string{
				...
				`  rm $tFile`,
				`}`,
				`local file="$(type a1 | head -n 1 | grep "is aliased to.*_custom_execute_" | grep "_custom_execute_[^[:space:]]*" -o | sed s/_custom_execute_//g)"`,
				`if [ -z "$file" ]; then`,
				`  echo Provided CLI "a1" is not a CLI generated with github.com/leep-frog/command`,
				`  return 1`,
				`fi`,
				...
				`  local tFile=$(mktemp)`,
				`  $GOPATH/bin/_${file}_runner autocomplete do $COMP_POINT "$COMP_LINE" "some" "stuff" > $tFile`,
				`  local IFS='`,
				`';`,
				`  COMPREPLY=( $(cat $tFile) )`,
				`  rm $tFile`,
				...
				`local file="$(type otherAlias | head -n 1 | grep "is aliased to.*_custom_execute_" | grep "_custom_execute_[^[:space:]]*" -o | sed s/_custom_execute_//g)"`,
				`if [ -z "$file" ]; then`,
				`  echo Provided CLI "otherAlias" is not a CLI generated with github.com/leep-frog/command`,
				`  return 1`,
				`fi`,
				`alias -- otherAlias="all args --at once "`,
				`function _custom_autocomplete_for_alias_otherAlias {`,
				`  local file="$(type all args --at once | head -n 1 | grep "is aliased to.*_custom_execute_" | grep "_custom_execute_[^[:space:]]*" -o | sed s/_custom_execute_//g)"`,
				`  local tFile=$(mktemp)`,
				`  $GOPATH/bin/_${file}_runner autocomplete all args --at once $COMP_POINT "$COMP_LINE"  > $tFile`,
				`  local IFS='`,
				`';`,
				`  COMPREPLY=( $(cat $tFile) )`,
				`  rm $tFile`,
