				...
				osChecks: map[string]*osCheck{
					...
					osWindows: {
						...
						wantStdout: []string{
							...
							`}`,
							``,
							`if (!(Test-Path alias:g) -or !(Get-Alias do | where {$_.DEFINITION -match "_custom_execute"}).NAME) {`,
							...
							`if (!(Test-Path alias:g) -or !(Get-Alias flaggable | where {$_.DEFINITION -match "_custom_execute"}).NAME) {`,
