	...
	for _, test := range []struct {
		...
		},
		{
			...
			wantFile: []string{
				...
				`  story`,
				`	}`,
				`	alias basic='o=$(mktemp) && _setup_for_basic_cli > $o && _custom_execute basic $o'`,
				"complete -F _custom_autocomplete -o nosort basic",
				`alias l='_custom_execute l'`,
				"complete -F _custom_autocomplete -o nosort l",
				"alias x='_custom_execute x'",
				"complete -F _custom_autocomplete -o nosort x",
				"",
			},
		},
	} {
...
func TestSourcerer(t *testing.T) {
	...
	for _, test := range []struct {
	...
	}{
		{
			...
			args: []string{"wizardry"},
			wantStderr: []string{
				"Unprocessed extra args: [wizardry]",
				u,
			},
		},
		// Execute tests
