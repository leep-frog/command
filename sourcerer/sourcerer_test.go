					...
					osLinux: {
						wantStdout: []string{
							...
							color.Apply(strings.Join([]string{
								...
								`  pushd . > /dev/null`,
								`  cd "\\fake\\source"`,
								fmt.Sprintf(`  go run . source "leepFrogSource" %q`, testutil.FilepathAbs(t, "cmd")),
								`  popd . > /dev/null`,
								`}`,
							}, "\n"), color.Blue),
						},
					...
					osLinux: {
						wantStdout: []string{
							...
							color.Apply(strings.Join([]string{
								...
								`  pushd . > /dev/null`,
								`  cd "\\fake\\source"`,
								fmt.Sprintf(`  go run . source "customOutputFile" %q`, testutil.FilepathAbs(t, "cmd")),
								`  popd . > /dev/null`,
								`}`,
							}, "\n"), color.Blue),
						},
				...
				osChecks: map[string]*osCheck{
					osLinux: {
						wantStdout: []string{
							...
							color.Apply(strings.Join([]string{
								`# Load all of your CLIs`,
								fmt.Sprintf(`source %q`, testutil.FilepathAbs(t, `cmd`, `leepFrogBuiltIns_loader.sh`)),
								``,
								`# Useful function to easily regenerate all of your CLIs whenever your go code changes`,
								`function _regenerate_leepFrogBuiltIns_CLIs() {`,
								`  pushd . > /dev/null`,
								`  cd "\\fake\\source"`,
								fmt.Sprintf(`  go run . builtin source "leepFrogBuiltIns" %q`, testutil.FilepathAbs(t, "cmd")),
								`  popd . > /dev/null`,
								`}`,
							}, "\n"), color.Blue),
						},
					...
					osWindows: {
						wantStdout: []string{
							...
							color.Apply(strings.Join([]string{
								`# Load all of your CLIs`,
								fmt.Sprintf(`. %q`, testutil.FilepathAbs(t, "cmd", "leepFrogBuiltIns_loader.ps1")),
								``,
								`# Useful function to easily regenerate all of your CLIs whenever your go code changes`,
								`function _regenerate_leepFrogBuiltIns_CLIs() {`,
								`  Push-Location`,
