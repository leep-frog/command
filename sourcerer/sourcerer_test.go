	...
	}{
		{
			...
			osChecks: map[string]*osCheck{
				...
				osWindows: {
					...
					wantStdout: []string{
						`pushd`,
						`cd "$(Split-Path /fake/source/location)"`,
			...
			osChecks: map[string]*osCheck{
				...
				osWindows: {
					...
					wantStdout: []string{
						...
						`  _setup_for_basic_cli -and _custom_execute_leep-frog-source basic $Local:setupTpmFile $args`,
						...
						``,
						`  # Run the go-only code`,
						`  & $env:GOPATH/bin/_leep-frog-source_runner.exe execute "l" $tmpFile $args`,
						`  # Return error if failed`,
						`  If (!$?) { throw "Go execution failed" }`,
						``,
						`  # If success, run the ExecuteData.Executable data`,
						...
						``,
						`  # Run the go-only code`,
						`  & $env:GOPATH/bin/_leep-frog-source_runner.exe execute "x" $tmpFile $args`,
						`  # Return error if failed`,
						`  If (!$?) { throw "Go execution failed" }`,
						``,
						`  # If success, run the ExecuteData.Executable data`,
		...
		for _, curOS := range []OS{Linux() /*, Windows()*/} {
			t.Run(fmt.Sprintf("[%s] %s", curOS.Name(), test.name), func(t *testing.T) {
				...
				if !oschk.noStderrNewline {
					oschk.wantStderr = append(oschk.wantStderr, "")
				}
				if !oschk.noStdoutNewline {
					oschk.wantStdout = append(oschk.wantStdout, "")
				}
				if diff := cmp.Diff(strings.Join(oschk.wantStdout, "\n"), o.GetStdout()); diff != "" {
