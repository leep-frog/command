...
func (l *linux) completeScriptCommand(functionName, alias string) string {
	return fmt.Sprintf(`((type complete > /dev/null 2>&1) && complete -F %s %s %s) || (echo 'shell function "complete" either failed or does not exist; if using zsh, be sure to set up bashcompinit')`, functionName, NosortString(), alias)
}
