		...
		},
		{
			...
			etc: &ExecuteTestCase{
				Node: &BranchNode{
					...
					Default: SerialNodes(ListArg[string]("sl", testDesc, 0, UnboundedList), printArgsNode()),
				},
...
func TestComplete(t *testing.T) {
	...
	for _, test := range []struct {
		...
		// Flag completion with list breaker
		{
			...
			ctc: &CompleteTestCase{
				Node: SerialNodes(
					ListArg[string]("lst", testDesc, 0, UnboundedList, ConvertListBreaker[[]string](breakerFlagProcessor.ListBreaker())),
...
func TestPanics(t *testing.T) {
	for _, test := range []struct {
		...
		},
		{
			...
			f: func() {
				Arg[*SimpleNode]("n", testDesc).Execute(NewInput([]string{"abc"}, nil), NewFakeOutput(), &Data{}, &ExecuteData{})
			},
